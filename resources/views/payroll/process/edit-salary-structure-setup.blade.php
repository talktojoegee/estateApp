...
@section('main-content')
    ...
    <div class="row">
        <div class="col-md-12 col-xl-12 col-sm-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('update-salary-structure') }}" data-parsley-validate="" method="post" autocomplete="off" id="addPropertyForm" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="modal-header text-uppercase mb-3">Edit Salary Structure
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-md-6 lg-6">
                                    <div class="form-group">
                                        ...
                                        <select name="salaryStructureType" id="salaryStructureType" class="form-control">
                                            ...
                                            <option value="0" {{ $user->salary_structure_category == 0 ? 'selected' : null }}>Personalized</option>
                                            <option value="1" {{ $user->salary_structure_category != 0 ? 'selected' : null }}>Categorized</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="category" data-parsley-required-message="Select salary structure" required  class="form-control p-3 select2" name="category">
                                            ...
                                            @foreach($categories as $record)
                                                <option value="{{$record->id}}" {{ $user->salary_structure_category == $record->id ? 'selected' : null }}> {{ $record->ss_name ?? '' }} </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row" id="guard">
                                <div class="col-sm-12 col-md-12 lg-12" id="appendList">
                                    <div id="items">
                                        @foreach($user->getEmployeePersonalizedSalaryStructures as $struct)
                                            <div class="row item mt-3">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select name="paymentDefinition[]" id="" class="form-control ">
                                                            @foreach($definitions as $option)
                                                                <option value="{{$option->id}}" {{$struct->payment_definition_id == $option->id ? 'selected' : null }}>{{ $option->payment_name ?? ''  }} ({{ $option->pay_code ?? '' }})</option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="col-md-5">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" step="0.01" value="{{ $struct->amount ?? 0 }}" placeholder="Amount" name="amount[]" class="form-control">
                                                        @error('amount')
                                                        <i class="text-danger mt-2">{{$message}}</i>
                                                        @enderror
                                                    </div>
...
@section('extra-scripts')
    ...
    <script>
        $(document).ready(function(){
            let structureType = "{{ $user->salary_structure_category }}"
            if(parseInt(structureType) == 0){
                $('#guard').show();
                $('#category').hide();
            }else{
                $('#guard').hide();
